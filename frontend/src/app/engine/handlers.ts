// File: frontend/src/app/engine/handlers.ts
...
async function setBackgroundFromFile(file: File, core: Core): Promise<void> {
  ...
  reader.onload = () => {
    ...
    img.onload = async () => {
      const ext = core.utils.getFileExtension(file);
      const assetType: AssetType = core.utils.getAssetType(imgUrl, ext);
      const hash_sha256 = await core.utils.getFileSHA256(file);
      const { width, height } = await core.utils.getImageDimensions(file);
      ...
      const asset = {
        ...
        ext,
        tags: [],
        size_kb: core.utils.getFileSizeInKB(file),
        hash_sha256,
